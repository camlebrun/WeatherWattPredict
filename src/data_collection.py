...
# URL Template for the first dataset
URL_TEMPLATE_1 = (
...
# URL Template for the second dataset
URL_TEMPLATE_2 = (
    "https://object.files.data.gouv.fr/meteofrance/data/synchro_ftp/BASE/"
    "QUOT/Q_{departement}_latest-2022-2023_RR-T-Vent.csv.gz"
)
...
for departement_number in range(1, 99):
    ...
    if departement_number != 62:
        # Formater l'URL avec le numéro du département pour le premier dataset
        url_1 = URL_TEMPLATE_1.format(departement=str(departement_number).zfill(2))

        # Formater l'URL avec le numéro du département pour le second dataset
        url_2 = URL_TEMPLATE_2.format(departement=str(departement_number).zfill(2))
                ...
                with BytesIO(response.content) as compressed_file:
                    ...
                    with gzip.open(compressed_file, 'rt') as csv_file:
                        try:
                            ...
                            print(f"Erro on  {departement_number}: {e}")

            else:
                # Print un message en cas d'échec de téléchargement
                print(f"Fail for {departement_number}.  HTTP : {response.status_code}")

# Définir le chemin du fichier de sortie dans le répertoire de destination
output_file_path = os.path.join(DESTINATION_DIRECTORY, "data_departements_merged.csv")
